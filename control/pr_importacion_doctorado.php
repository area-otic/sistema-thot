...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['archivo'])) {
    try {
        ...
        header('Location: ../pages/gestion_doctorado.php?success='.urlencode("Archivo importado correctamente. Registros procesados: $registros"));
        exit();
        
    } catch (Exception $e) {
        error_log('Error al importar: ' . $e->getMessage());
        header('Location: ../pages/importar_doctorado.php?error=1&msg=' . urlencode($e->getMessage()));
        exit();
    }
} else {
    header('Location: ../pages/importar_doctorado.php?error=1&msg=No se subió ningún archivo');
    exit();
}
...
function procesarCSV($ruta_archivo, $fecha_actual) {
    ...
    $sql = "INSERT INTO data_doctorados (
